//
//  UserInfoMakeUpViewController.m
//  TbeaCloudBusiness
//
...
#import "UserInfoMakeUpViewController.h"

@interface UserInfoMakeUpViewController ()

@end

@implementation UserInfoMakeUpViewController
...
-(void)clickgetmakelist   //选择隶属关系
{
    ...
    for(int i=0;i<[FCarraymakeuplist count];i++)
    {
        NSDictionary *dicuser = [FCarraymakeuplist objectAtIndex:i];
        [arrayuser addObject:[dicuser objectForKey:@"name"]];
    }
    [ZJBLStoreShopTypeAlert showWithTitle:@"请选择隶属关系" titles:arrayuser deleGate1:self selectIndex:^(NSInteger selectIndex) {
    } selectValue:^(NSString *selectValue) {
        DLog(@"selectvalue====%@",selectValue);
        UITextField *textfield = [tableview viewWithTag:EnUserMakeUpTextfield4];
        textfield.text = selectValue;
         FCcompanyid = [AddInterface returnselectid:FCarraymakeuplist SelectValue:selectValue];
    } showCloseButton:YES];
}
...
-(void)picupload
{
    UIImageView *imageview = [tableview viewWithTag:EnUserMakeUpImageView1];
...
{
    UITextField *textfield1 = [tableview viewWithTag:EnUserMakeUpTextfield5];
    FCrealname = textfield1.text;
    
    if(([FCrealname length]==0)||([FCprovice length]==0)||([FCyear length]==0)||([FCsex length]==0)||([FCcompanyid length]==0)||([FCuploadstrpic length]==0))
    {
...
-(void)getmakeuptype
{
    ...
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQUserInfoMakeUpViewLiShuGuanXiCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        
    } Success:^(NSDictionary *dic) {
        DLog(@"dic====%@",dic);
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            FCarraymakeuplist = [[dic objectForKey:@"data"] objectForKey:@"distributorlist"];
        }
        else
        {
...
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"mobilenumber"] = app.userinfo.usertel;
    params[@"usertypeid"] = app.userinfo.usertype;
    params[@"province"] = FCprovice;
    params[@"city"] = FCcity;
    params[@"zone"] = FCarea;
    params[@"uplevelcompanyid"] = FCcompanyid;
    params[@"sexid"] = FCsex;
    params[@"realname"] = FCrealname;
    params[@"birthYear"] = FCyear;
    params[@"birthmonth"] = FCmonth;
    params[@"birthday"] = FCday;
    params[@"picture"] = FCuploadstrpic;
    
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQUserInfoMakeUpAddUserInfoCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
