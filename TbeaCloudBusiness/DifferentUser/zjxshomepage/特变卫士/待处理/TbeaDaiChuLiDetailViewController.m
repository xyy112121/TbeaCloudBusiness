//
//  TbeaDaiChuLiDetailViewController.m
//  TbeaCloudBusiness
//
...
#import "TbeaDaiChuLiDetailViewController.h"

@interface TbeaDaiChuLiDetailViewController ()

@end

@implementation TbeaDaiChuLiDetailViewController
...
-(void)viewfoottagview
{
    ...
    [button setTitle:@"派单" forState:UIControlStateNormal];
    button.titleLabel.font = FONTN(15.0f);
    button.layer.cornerRadius = 3.0f;
    button.clipsToBounds = YES;
    [button addTarget:self action:@selector(clickpaidan:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)clickpaidan:(id)sender
{
    TbeaPaiDanPersonListViewController *paidanperson = [[TbeaPaiDanPersonListViewController alloc] init];
    paidanperson.delegate1 = self;
    [self.navigationController pushViewController:paidanperson animated:YES];
    ...
    if(indexPath.section == 0)
    {
        switch (indexPath.row)
        {
            ...
            case 6:
                ...
                labelvalue.text = [[FCdicdata objectForKey:@"electricalcheckinfo"] objectForKey:@"customeraddress"];
                [cell.contentView addSubview:labelvalue];
                break;
        }
    }
    else
    {
        switch (indexPath.row)
        {
            ...
            case 2:
                ...
                labelvalue.text = [[FCdicdata objectForKey:@"orderinfo"] objectForKey:@"ordercompanyid"];
                [cell.contentView addSubview:labelvalue];
                break;
            case 3:
                labelname.text = @"备注";
...
#pragma mark 接口
//获取详情
-(void)getdaichulidetailinfo
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"electricalcheckid"] = self.FCelectricalcheckid;
    
    
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQTbeaDaiChuLiDetailInfoCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
