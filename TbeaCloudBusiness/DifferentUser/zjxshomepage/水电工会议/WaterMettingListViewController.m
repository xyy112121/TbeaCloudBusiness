//
//  WaterMettingListViewController.m
//  TbeaCloudBusiness
//
...
#import "WaterMettingListViewController.h"

@interface WaterMettingListViewController ()

@end

@implementation WaterMettingListViewController
...
-(void)initview
{
    ...
    tableview.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        [weakSelf getmettinglist:@"1" PageSize:@"10"];
    }];
    
    tableview.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [weakSelf getmettinglist:@"1" PageSize:[NSString stringWithFormat:@"%d",(int)[FCarraydata count]+10]];
    }];
    // 默认先隐藏footer
...
-(void)dropDownListParame:(NSString *)aStr
{
	flagnow = 0;
	DLog(@"astr====%@",aStr);
	if(enselectitem == EnWaterMettingmetting)
	{
		
	}
	else if(enselectitem == EnWaterMettingarea)
	{
...
-(void)getmettinglist:(NSString *)page PageSize:(NSString *)pagesize
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    ...
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQWaterMettingListCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
