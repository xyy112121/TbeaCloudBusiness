...
- (void)pickerTimer:(FDPickerTimer *)pickerTimer year:(NSInteger)year month:(NSInteger)month day:(NSInteger)day{
	...
	UITextField *textfield = [self.view viewWithTag:enselecttime==EnStartTime?EnTimeSelectTextfieldtag1:EnTimeSelectTextfieldtag2];
	textfield.text = text;
...
- (void)pickerAreaer:(FDCityPicker *)pickerAreaer province:(NSString *)province city:(NSString *)city area:(NSString *)are
{
...
-(void)clickdone:(id)sender
{
	...
	else
	{
        ...
        UITextField *textfield1 = [self.view viewWithTag:EnTimeSelectTextfieldtag1];
        UITextField *textfield2 = [self.view viewWithTag:EnTimeSelectTextfieldtag2];
        DLog(@"textfield====%@,%@",textfield1.text,textfield2.text);
        if([textfield1.text compare:textfield2.text] == NSOrderedAscending)
        {
            if([self.delegate1 respondsToSelector:@selector(DGSelectDateDone:EndTime:)])
            {
                [self.delegate1 DGSelectDateDone:textfield1.text EndTime:textfield2.text];
            }
            [self returnback];
        }
        else
