//
//  ScanSelectCommityViewController.m
//  TbeaCloudBusiness
//
...
#import "ScanSelectCommityViewController.h"

@interface ScanSelectCommityViewController ()

@end

@implementation ScanSelectCommityViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UILabel *labelname = [[UILabel alloc] initWithFrame:CGRectMake(20, 10, SCREEN_WIDTH-30, 20)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([self.delegate1 respondsToSelector:@selector(DGProductSelectCommity:)])
    {
        [self.delegate1 DGProductSelectCommity:dictemp];
    }
...
#pragma mark 接口
//获取产品名称
-(void)getcommitynamelist
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    
    
    
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQCreateGetCommityListCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        
    } Success:^(NSDictionary *dic) {
        DLog(@"dic====%@",dic);
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            FCarraydata = [[dic objectForKey:@"data"] objectForKey:@"tbeacommoditycaterogylist"];
