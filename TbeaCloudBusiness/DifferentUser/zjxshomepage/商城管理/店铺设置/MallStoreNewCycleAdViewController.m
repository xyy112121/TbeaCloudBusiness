//
//  MallStoreNewCycleAdViewController.m
//  TbeaCloudBusiness
//
...
#import "MallStoreNewCycleAdViewController.h"

@interface MallStoreNewCycleAdViewController ()

@end

@implementation MallStoreNewCycleAdViewController
...
-(UIView *)viewcellcommdity:(NSDictionary *)sender Frame:(CGRect)frame
{
    ...
    CGSize size = [AddInterface getlablesize:[sender objectForKey:@"name"] Fwidth:SCREEN_WIDTH-170 Fheight:45 Sfont:FONTN(15.0f)];
...
-(UIView *)viewcellactivity:(NSDictionary *)sender Frame:(CGRect)frame
{
    ...
    CGSize size = [AddInterface getlablesize:[sender objectForKey:@"title"] Fwidth:SCREEN_WIDTH-160 Fheight:40 Sfont:FONTN(15.0f)];
    UILabel *labeltitle = [[UILabel alloc] initWithFrame:CGRectMake(XYViewR(imageview)+10,XYViewTop(imageview),size.width,size.height)];
    labeltitle.backgroundColor = [UIColor clearColor];
    labeltitle.textColor = [UIColor blackColor];
    labeltitle.font = FONTN(15.0f);
...
-(void)uploadcyclead
{
    ...
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQStoreAddnewCycleadInfoCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
