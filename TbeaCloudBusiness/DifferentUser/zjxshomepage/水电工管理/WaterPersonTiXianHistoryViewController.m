//
//  WaterPersonTiXianHistoryViewController.m
//  TbeaCloudBusiness
//
...
#import "WaterPersonTiXianHistoryViewController.h"

@interface WaterPersonTiXianHistoryViewController ()

@end

@implementation WaterPersonTiXianHistoryViewController
...
-(void)initview
{
    ...
    tableview.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        [weakSelf getwatertixianhistorylist:@"1" Pagesize:@"10"];
    }];
    
    tableview.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [weakSelf getwatertixianhistorylist:@"1" Pagesize:[NSString stringWithFormat:@"%d",(int)[FCarraydata count]+10]];
    }];
    // 默认先隐藏footer
...
-(UIView *)getviewselectitem:(CGRect)frame
{
  ...
  //  [buttonitemarea.button addTarget:self action:@selector(ClickSelectMoney:) forControlEvents:UIControlEventTouchUpInside];
    buttonitemarea.tag = EnTixianDataSelectItembt2;
    [buttonitemarea updatebuttonitem:EnButtonTextCenter TextStr:@"扫码地点" Font:FONTN(14.0f) Color:COLORNOW(117, 117, 117) Image:nil];
...
-(void)jingxiaoshangheader:(UIView *)viewheader
{
    ...
    NSString *strname = [FCdicelectricianinfo objectForKey:@"personname"];
    ...
    NSString *stricon = [FCdicelectricianinfo objectForKey:@"persontypeicon"];//[InterfaceResource stringByAppendingString:[[FCdicelectricianinfo objectForKey:@"persontypeicon"] length]>0?[FCdicelectricianinfo objectForKey:@"persontypeicon"]:@"noimage.png"];
    ...
    lableaddr.text =[FCdicelectricianinfo objectForKey:@"zone"];
    ...
    buttonperson.backgroundColor = [UIColor clearColor];
    [buttonperson addTarget:self action:@selector(clickpersonhp:) forControlEvents:UIControlEventTouchUpInside];
    [viewheader addSubview:buttonperson];
...
-(void)wateruserheader:(UIView *)viewheader
{
    ...
    strpic = [FCdicelectricianinfo objectForKey:@"persontypeicon"];//[InterfaceResource stringByAppendingString:[[FCdicelectricianinfo objectForKey:@"persontypeicon"] length]>0?[FCdicelectricianinfo objectForKey:@"persontypeicon"]:@"noimage.png"];
    [imageheader setImageWithURL:[NSURL URLWithString:strpic]];
    [viewheader addSubview:imageicon];
    
    
    NSString *strmoney = [NSString stringWithFormat:@"%@",[FCdicelectricianinfo objectForKey:@"totlemoney"]];
    CGSize size2 = [AddInterface getlablesize:strmoney Fwidth:120 Fheight:20 Sfont:FONTMEDIUM(17.0f)];
    UILabel *lablemoneyvalue = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-size2.width-10, 20, size2.width, 20)];
    lablemoneyvalue.text =strmoney;
    lablemoneyvalue.font = FONTMEDIUM(17.0f);
...
-(void)clickpersonhp:(id)sender
{
    ...
    waterperson.FCelectricianid = [FCdicelectricianinfo objectForKey:@"userid"];
...
#pragma mark 接口
//获取列表
-(void)getwatertixianhistorylist:(NSString *)page Pagesize:(NSString *)pagesize
{
    ...
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:REWaterTiXianHistoryListCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        ...
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            ...
            FCdicelectricianinfo = [[dic objectForKey:@"data"] objectForKey:@"electricianinfo"];
            [self addtabviewheader];
