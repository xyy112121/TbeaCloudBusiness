//
//  TbeaDetectUserHPViewController.m
//  TbeaCloudBusiness
//
...
#import "TbeaDetectUserHPViewController.h"

@interface TbeaDetectUserHPViewController ()

@end

@implementation TbeaDetectUserHPViewController
...
-(void)initview
{
    ...
    FCinterfacecode = @"TBEAYUN012001002000";
    FCorderitemvalue = @"";
    FCorderid = @"desc";
    FCordercode = @"";
    FCorderdate = @"";
...
-(UIView *)viewselectitem:(CGRect)frame
{
    ...
    if([FCselectitemid isEqualToString:@"assigned"])
    {
        ...
        [buttonitemstatus updatebuttonitem:EnButtonTextCenter TextStr:@"业主" Font:FONTN(14.0f) Color:COLORNOW(117, 117, 117) Image:LOADIMAGE(@"arrawgray", @"png")];
    ...
    else if([FCselectitemid isEqualToString:@"accepted"])
    {
        
        [buttonitemcode updatebuttonitem:EnButtonTextLeft TextStr:@"预约编号" Font:FONTN(14.0f) Color:COLORNOW(117, 117, 117) Image:LOADIMAGE(@"arrawgray", @"png")];
        buttonitemstatus.alpha = 0;
//        buttonitemstatus.frame = CGRectMake(10+widthnow*5, XYViewBottom(line1), widthnow*4, 40);
//        [buttonitemstatus updatebuttonitem:EnButtonTextCenter TextStr:@"业主" Font:FONTN(14.0f) Color:COLORNOW(117, 117, 117) Image:LOADIMAGE(@"arrawgray", @"png")];
        [buttonitemdate updatebuttonitem:EnButtonTextRight TextStr:@"接单日期" Font:FONTN(14.0f) Color:COLORNOW(117, 117, 117) Image:LOADIMAGE(@"arrawgray", @"png")];
    }
...
-(void)DGCommitClickJieDan:(NSString *)strelecid
{
    [self comfirmjiedan:strelecid];
}
...
-(void)clicktbeatype:(id)sender
{
    ...
    if([FCselectitemid isEqualToString:@"all"])  //全部
    {
        FCinterfacecode = @"TBEAYUN012001002000";
        [self gettbealist:@"1" Pagesize:@"10" InterfaceCode:FCinterfacecode];
    }
    else if([FCselectitemid isEqualToString:@"assigned"])  //待处理
    {
        FCinterfacecode = @"TBEAYUN012001003000";
        [self gettbealist:@"1" Pagesize:@"10" InterfaceCode:FCinterfacecode];
    }
    else if([FCselectitemid isEqualToString:@"accepted"]) //已接单
    {
        FCinterfacecode = @"TBEAYUN012001004000";
        [self gettbealist:@"1" Pagesize:@"10" InterfaceCode:FCinterfacecode];
    }
    else if([FCselectitemid isEqualToString:@"finishedbutneedappraise"]) //已完工
    {
        FCinterfacecode = @"TBEAYUN012001005000";
        [self gettbealist:@"1" Pagesize:@"10" InterfaceCode:FCinterfacecode];
    }
    else if([FCselectitemid isEqualToString:@"finished"])  //已上传
    {
        FCinterfacecode = @"TBEAYUN012001006000";
        [self gettbealist:@"1" Pagesize:@"10" InterfaceCode:FCinterfacecode];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([FCselectitemid isEqualToString:@"assigned"])
    {
        lablearea.text = [dictemp objectForKey:@"customername"];
        lablemoneyvalue.text = [dictemp objectForKey:@"subscribetime"];
    }
    else if([FCselectitemid isEqualToString:@"accepted"])
    {
        lablemoneyvalue.text = [dictemp objectForKey:@"accepttime"];
    }
    else if([FCselectitemid isEqualToString:@"finishedbutneedappraise"])
    {
...
{
    NSDictionary *dictemp = [FCarraydata objectAtIndex:indexPath.row];
    if([[dictemp objectForKey:@"checkstatusid"] isEqualToString:@"assigned"])  //对于检测人员来说是未接单
    {
        TbeaDetectUserWeiJieDanViewController *yipaidan = [[TbeaDetectUserWeiJieDanViewController alloc] init];
        yipaidan.FCelectricalcheckid = [dictemp objectForKey:@"electricalcheckid"];
        yipaidan.delegate1 = self;
        [self.navigationController pushViewController:yipaidan animated:YES];
    }
    else if([[dictemp objectForKey:@"checkstatusid"] isEqualToString:@"accepted"])
    {
        TbeaDetectUserYiJieDanViewController *yipandan = [[TbeaDetectUserYiJieDanViewController alloc] init];
        yipandan.FCelectricalcheckid = [dictemp objectForKey:@"electricalcheckid"];
...
-(void)gettbeatype
{
    ...
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQTbeaDetectUserHpSelectItemTypeCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        ...
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            ...
            [self gettbealist:@"1" Pagesize:@"10" InterfaceCode:@"TBEAYUN012001002000"];//全部
...
-(void)comfirmjiedan:(NSString *)eleccheckid
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"electricalcheckid"] = eleccheckid;
    
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQTbeaDetectUserJieDanDoneCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        
    } Success:^(NSDictionary *dic) {
        DLog(@"dic====%@",dic);
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            TbeaDetectUserWeiJieDanDoneViewController *paidandone = [[TbeaDetectUserWeiJieDanDoneViewController alloc] init];
            paidandone.FCelectricalcheckid = eleccheckid;
