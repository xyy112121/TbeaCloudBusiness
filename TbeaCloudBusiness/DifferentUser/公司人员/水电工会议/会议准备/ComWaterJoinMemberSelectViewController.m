//
//  ComWaterJoinMemberSelectViewController.m
//  TbeaCloudBusiness
//
...
#import "ComWaterJoinMemberSelectViewController.h"

@interface ComWaterJoinMemberSelectViewController ()

@end

@implementation ComWaterJoinMemberSelectViewController
...
-(void)viewcell1:(NSDictionary *)dicfrom FromCell:(UITableViewCell *)fromcell IndexTag:(int)indextag
{
    ...
    CGSize sizeuser = [AddInterface getlablesize:[dicfrom objectForKey:@"personname"] Fwidth:200 Fheight:20 Sfont:FONTMEDIUM(16.0f)];
    ...
    labelusername.text = [dicfrom objectForKey:@"personname"];
    ...
    straddr.text = [dicfrom objectForKey:@"cityzone"];
...
-(void)returnback
{
    if([self.delegate1 respondsToSelector:@selector(DGSelectJoinMemberCellView:)])
    {
        [self.delegate1 DGSelectJoinMemberCellView:FCarrayselectdata];
    }
...
-(int)judgearraycontaindic:(NSMutableArray *)arraydata DicSrc:(NSDictionary *)dicsrc
{
    ...
    for(int i=0;i<[FCarrayselectdata count];i++)
    {
        ...
        if([[dicselect objectForKey:@"userid"] isEqualToString:[dicsrc objectForKey:@"userid"]])
        {
            flag = 1;
            break;
        }
...
-(void)getjoinmemberlist
{
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"companyid"] = @"";
    params[@"userworkrole"] = @"market";
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQComWaterMettingJoinMemberListCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        
    } Success:^(NSDictionary *dic) {
        DLog(@"dic====%@",dic);
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            FCarraydata  = [[dic objectForKey:@"data"] objectForKey:@"companyemployeelist"];
            NSArray *arrayselect = [self.FCjoinmemberids componentsSeparatedByString:@","];
            for(int i=0;i<[FCarraydata count];i++)
            {
                NSDictionary *dictmep = [FCarraydata objectAtIndex:i];
                for(int j=0;j<[arrayselect count];j++)
                {
                    NSString *strid = [arrayselect objectAtIndex:j];
                    if([strid isEqualToString:[dictmep objectForKey:@"userid"]])
                    {
                        [FCarrayselectdata addObject:dictmep];
                        break;
                    }
                }
            }
