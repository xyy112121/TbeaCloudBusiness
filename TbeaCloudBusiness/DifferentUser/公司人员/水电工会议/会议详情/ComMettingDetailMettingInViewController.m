//
//  ComMettingDetailMettingInViewController.m
//  TbeaCloudBusiness
//
...
@interface ComMettingDetailMettingInViewController ()

@end

@implementation ComMettingDetailMettingInViewController
...
{
    if([TITLES[index] isEqualToString:@"更新"])
    {
        ComMettingDetailUploadViewController *upload = [[ComMettingDetailUploadViewController alloc] init];
    ...
    else if(indexPath.section == 2)
    {
        switch (indexPath.row)
        {
            ...
            case 1:
                ...
                labelvalue.text = FCmettingsummary;
                [cell.contentView addSubview:labelvalue];
                break;
            case 2:
                cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
                ...
                labelvalue.text = FCmettingpicnumber;
                labelvalue.textAlignment = NSTextAlignmentRight;
                labelvalue.frame = CGRectMake(SCREEN_WIDTH-30-XYViewWidth(labelvalue), XYViewTop(labelvalue), XYViewWidth(labelvalue), XYViewHeight(labelvalue));
                [cell.contentView addSubview:labelvalue];
                break;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(editstatus == EnMettingNotEdit)
    {
        ...
        ComMettinDetailUploadSummaryViewController *mettingsummary; //会议纪要
        ComMettingDetailSHowPicViewController *detailshowpic;
            ...
            case 2:
                switch (indexPath.row)
                {
                    case 0:
                        ...
                        break;
                    case 1:
                        mettingsummary = [[ComMettinDetailUploadSummaryViewController alloc] init];
                        mettingsummary.FCmettingid = [[FCdicdata objectForKey:@"meetingbaseinfo"] objectForKey:@"id"];
                        mettingsummary.FCmettingdes = [[FCdicdata objectForKey:@"meetinginfo"] objectForKey:@"meetingsummary"];
                        mettingsummary.FCfromflag = @"0";
                        [self.navigationController pushViewController:mettingsummary animated:YES];
                        break;
                    case 2:
                        detailshowpic = [[ComMettingDetailSHowPicViewController alloc] init];
                        detailshowpic.FCmettingid = [[FCdicdata objectForKey:@"meetingbaseinfo"] objectForKey:@"id"];
                        detailshowpic.FCfromflag = @"0";
                        [self.navigationController pushViewController:detailshowpic animated:YES];
                        break;
                }
...
-(void)getmettingdetail
{
    ...
    [RequestInterface doGetJsonWithParametersNoAn:params App:app RequestCode:RQWaterMettingDetailCode ReqUrl:InterfaceRequestUrl ShowView:app.window alwaysdo:^{
        ...
        if([[dic objectForKey:@"success"] isEqualToString:@"true"])
        {
            ...
            FCmettingpicnumber = [NSString stringWithFormat:@"%@",[[FCdicdata objectForKey:@"meetinginfo"] objectForKey:@"meetingpicturenumber"]];
            FCmettingsummary = [NSString stringWithFormat:@"%@",[[FCdicdata objectForKey:@"meetinginfo"] objectForKey:@"meetingsummary"]];
