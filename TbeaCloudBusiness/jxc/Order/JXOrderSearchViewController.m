...
#import "YTextField.h"
@interface JXOrderSearchViewController ()

@end

@implementation JXOrderSearchViewController
...
-(void)initview:(id)sender
{
	...
	textfieldusername.placeholder = @"订单号查询";
...
{
	YTextField *textfieldusername = (YTextField *)[self.view viewWithTag:101];
	if([textfieldusername.text length]==0)
	{
        [MBProgressHUD showError:@"请输入订单号!" toView:self.view];

	}
	else
	{
		if([delegate1 respondsToSelector:@selector(clicksearchcode:)])
		{
			[delegate1 clicksearchcode:textfieldusername.text];
		}
	}
	[self returnback:nil];
}
