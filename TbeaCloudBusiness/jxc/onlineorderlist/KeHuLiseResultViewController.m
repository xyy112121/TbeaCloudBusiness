//
//  KeHuLiseResultViewController.m
//  TeBian
//
...
@interface KeHuLiseResultViewController ()

@end

@implementation KeHuLiseResultViewController
...
-(void) requestkehulist:(NSString *)page Pagesize:(NSString *)pagesize Pcode:(NSString *)pcode Pname:(NSString *)pname Pdes:(NSString *)pdes
{

	
	NSDictionary *parameters =  @{@"CustomerCode":pcode,@"CustomerName":pname,@"Page":page,@"PageSize":pagesize};
	
	AppDelegate *app = (AppDelegate *)[[UIApplication sharedApplication] delegate];
	[RequestInterface doGetJsonWithParametersNoAn:parameters App:app RequestCode:@"TBEA07030800" ReqUrl:URLHeader ShowView:app.window alwaysdo:^
	 {
										  ...
										  Success:^(NSDictionary *responseObject)
	 {
		 ...
		 arraydata = [responseObject objectForKey:@"CustomerList"];
...
- (void)viewDidLoad {
	...
	[self requestkehulist:@"1" Pagesize:@"10" Pcode:self.strpcode Pname:self.strpname Pdes:self.strpdes];
    
    __weak __typeof(self) weakSelf = self;
    self.tableview.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        [weakSelf requestkehulist:@"1" Pagesize:@"10" Pcode:self.strpcode Pname:self.strpname Pdes:self.strpdes];
    }];
    
    self.tableview.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        [weakSelf requestkehulist:@"1" Pagesize:[NSString stringWithFormat:@"%d",(int)[arraydata count]+10] Pcode:self.strpcode Pname:self.strpname Pdes:self.strpdes];
    }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	labelcode.text = [dictemp objectForKey:@"CustomerCode"];
	...
	UILabel *labelname = [[UILabel alloc] initWithFrame:CGRectMake(labeltitle.frame.origin.x,imageviewline.frame.origin.y+10, 250, 20)];
	labelname.font = FONTN(12.0f);
	labelname.text = [NSString stringWithFormat:@"客户名称:  %@",[dictemp objectForKey:@"CustomerName"]];;
...
//	labelunit.text =  [NSString stringWithFormat:@"客户短名 %@",[dictemp objectForKey:@"CustomerShortName"]];// @"单位:KM";
...
//	[cell.contentView addSubview:labelunit];
	...
	labeldes.text = [NSString stringWithFormat:@"客户短名 %@",[dictemp objectForKey:@"CustomerShortName"]];
...
-(void)gotoaddwuliao:(id)sender
{
	...
	if([delegate1 respondsToSelector:@selector(updateselectkehu:)])
	{
		...
		[delegate1 updateselectkehu:dictemp];
		[self.navigationController popViewControllerAnimated:YES];
	}
}
